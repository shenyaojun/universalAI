...
_PROMPT_TEMPLATE = """
用户会提出一个需要你查询文档库的问题，你应该对问题进行理解和拆解，并在文档库中查询相关的内容。

对于每个文档库，你输出的内容应该是一个一行的字符串，这行字符串包含文档库名称和查询内容，中间用逗号隔开，不要有多余的文字和符号。你可以同时查询多个文档库，下面这个例子就是同时查询两个文档库的内容。
...
```text
${{文档库名称,查询问题,不要带有任何除了,之外的符号,比如不要输出中文的逗号，不要输出引号}}
    ...
    async def _aprocess_llm_result(
        ...
        if text_match:
            ...
            try:
                ...
                queries = [(line.split("，")[0].strip(), line.split("，")[1].strip()) for line in lines]
            await run_manager.on_text("文档库查询询内容:\n\n" + str(queries) + " \n\n", color="blue",
