...
def search_docs(
        ...
        knowledge_base_name: str = Body(..., description="文档库名称", examples=["samples"]),
        top_k: int = Body(VECTOR_SEARCH_TOP_K, description="匹配向量数"),
        score_threshold: float = Body(SCORE_THRESHOLD,
                                      description="文档库匹配相关度阈值，取值范围在0-1之间，"
                                                  "SCORE越小，相关度越高，"
...
def update_docs_by_id(
    ...
    kb = KBServiceFactory.get_service_by_name(knowledge_base_name)
    if kb is None:
        return BaseResponse(code=500, msg=f"指定的文档库 {knowledge_base_name} 不存在")
...
def list_files(
    ...
    kb = KBServiceFactory.get_service_by_name(knowledge_base_name)
    if kb is None:
        return ListResponse(code=404, msg=f"未找到文档库 {knowledge_base_name}", data=[])
    else:
...
#                 knowledge_base_name: str = Form(..., description="文档库名称", examples=["samples"]),
#                 override: bool = Form(False, description="覆盖已有文件"),
#                 save: bool = Form(True, description="是否将文件保存到文档库目录")):
...
def upload_docs(
        ...
        chunk_size: int = Form(CHUNK_SIZE, description="文档库中单段文本最大长度"),
        chunk_overlap: int = Form(OVERLAP_SIZE, description="文档库中相邻文本重合长度"),
    ...
    kb = KBServiceFactory.get_service_by_name(knowledge_base_name)
    if kb is None:
        return BaseResponse(code=404, msg=f"未找到文档库 {knowledge_base_name}")
...
def update_info(
        knowledge_base_name: str = Body(..., description="文档库名称", examples=["samples"]),
        kb_info: str = Body(..., description="文档库介绍", examples=["这是一个文档库"]),
):
    if not validate_kb_name(knowledge_base_name):
        return BaseResponse(code=403, msg="Don't attack me")
    ...
    return BaseResponse(code=200, msg=f"文档库介绍修改完成", data={"kb_info": kb_info})


def update_docs(
        knowledge_base_name: str = Body(..., description="文档库名称", examples=["samples"]),
        file_names: List[str] = Body(..., description="文件名称，支持多文件", examples=[["file_name1", "text.txt"]]),
        chunk_size: int = Body(CHUNK_SIZE, description="文档库中单段文本最大长度"),
        chunk_overlap: int = Body(OVERLAP_SIZE, description="文档库中相邻文本重合长度"),
...
def download_doc(
        knowledge_base_name: str = Query(..., description="文档库名称", examples=["samples"]),
...
def recreate_vector_store(
    ...
    def output():
        ...
        if not kb.exists() and not allow_empty_kb:
            yield {"code": 404, "msg": f"未找到文档库 ‘{knowledge_base_name}’"}
        else:
            if kb.exists():
                kb.clear_vs()
            kb.create_kb()
            ...
            for status, result in files2docs_in_thread(kb_files,
                ...
                if status:
                    ...
                    kb_name, file_name, error = result
                    msg = f"添加文件‘{file_name}’到文档库‘{knowledge_base_name}’时出错：{error}。已跳过。"
                    logger.error(msg)
                    yield json.dumps({
                        "code": 500,
                        "msg": msg,
