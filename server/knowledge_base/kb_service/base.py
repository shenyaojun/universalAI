...
class KBService(ABC):
    ...
    def __init__(self,
        ...
        self.kb_info = KB_INFO.get(knowledge_base_name, f"关于{knowledge_base_name}的文档库")
...
# add for pinyin shenyaojun added
...
def get_kb_details() -> List[Dict]:
    ...
    for kb in kbs_in_db:
        ...
        kb_pinyin = ''.join(lazy_pinyin(kb)) # shenyaojun added
        if kb_detail:
            kb_detail["in_db"] = True
            if kb in result:
                result[kb].update(kb_detail)
            else:  # shenyaojun added
                if kb_pinyin in result:  # shenyaojun added
                    del result[kb_pinyin]  # shenyaojun added
                    result[kb] = kb_detail # shenyaojun added
                else:
