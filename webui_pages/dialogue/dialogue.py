...
def dialogue_page(api: ApiRequest, is_lite: bool = False):
    ...
    if not chat_box.chat_inited:
        st.toast(
            f"欢迎使用 [`北京环球法务小助手`](https://www.universalbeijingresort.com/zh_CN) ! \n\n"
    ...
    with st.sidebar:
        ...
        def on_kb_change():
            st.toast(f"已加载文档库： {st.session_state.selected_kb}")

        if dialogue_mode == "文档库问答":
            with st.expander("文档库配置", True):
                ...
                kb_top_k = st.number_input("匹配资料条数：", 1, 20, VECTOR_SEARCH_TOP_K)

                ## Bge 模型会超过1
                score_threshold = st.slider("资料匹配分数阈值：", 0.0, 2.0, float(SCORE_THRESHOLD), 0.01)
        elif dialogue_mode == "文件对话":
            with st.expander("文件对话配置", True):
                files = st.file_uploader("上传资料文件：",
    ...
    if prompt := st.chat_input(chat_input_placeholder, key="prompt"):
        if parse_command(text=prompt, modal=modal): # 用户输入自定义命令
            ...
            elif dialogue_mode == "文档库问答":
                chat_box.ai_say([
                    f"正在查询文档库 `{selected_kb}` ...",
                    Markdown("...", in_expander=True, title="文档库匹配结果", state="complete"),
                ])
                text = ""
