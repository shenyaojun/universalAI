...
def knowledge_base_page(api: ApiRequest, is_lite: bool = None):
    try:
        ...
        st.error("获取文档库信息错误，请检查是否已按照 `README.md` 中 `4 文档库初始化与迁移` 步骤完成初始化或迁移，或是否为数据库连接错误。")
        st.stop()
    ...
    if selected_kb == "新建文档库":
        with st.form("新建文档库"):

            kb_name = st.text_input(
                "新建文档库名称",
                placeholder="新文档库名称，不支持中文命名",
                key="kb_name",
            )
            kb_info = st.text_input(
                "文档库简介",
                placeholder="文档库简介，方便Agent查找",
                key="kb_info",
            )
        ...
        if submit_create_kb:
            if not kb_name or not kb_name.strip():
                st.error(f"文档库名称不能为空！")
            elif kb_name in kb_list:
                st.error(f"名为 {kb_name} 的文档库已经存在！")
            else:
        ...
        kb_info = st.text_area("请输入文档库介绍:", value=st.session_state["selected_kb_info"], max_chars=None, key=None,
        ...
        if not len(doc_details):
            st.info(f"文档库 `{kb}` 中暂无文件")
        else:
            st.write(f"文档库 `{kb}` 中已有文件:")
            st.info("文档库中包含源文件与向量库，请从下表中选择文件后操作")
        ...
        # if cols[0].button(
        #         "依据源文件重建向量库",
        #         # help="无需上传文件，通过其它方式将文档拷贝到对应文档库content目录下，点击本按钮即可重建文档库。",
        #         use_container_width=True,
        #         type="primary",
        # ):
